...
class Music(commands.Cog):
    ...
    async def play_next(self, ctx_or_interaction, voice_client):
        ...
        embed = discord.Embed(title="🎵 Đang phát", description=f"**{source.title}**", color=discord.Color.pink())
        if source.thumbnail: embed.set_thumbnail(url=source.thumbnail)
        if source.duration: embed.add_field(name="Thời lượng", value=str(timedelta(seconds=source.duration)), inline=True)
    ...
    async def play(self, interaction: discord.Interaction, query: str):
        ...
        try:
            ...
            if voice_client.is_playing() or voice_client.is_paused():
                queue.append(source)
                embed = discord.Embed(title="➕ Đã thêm vào hàng chờ", description=f"**{source.title}**", color=discord.Color.green())
            ...
            if "YOUTUBE_403" in error_msg:
                msg = "😭 **YouTube đang chặn mạnh lắm rồi bé ơi!** Thử thêm cookies.txt hoặc dùng từ khóa tìm kiếm."
            else:
                msg = f"❌ Lỗi: {error_msg}"
    ...
    async def queue(self, interaction: discord.Interaction):
        ...
        if now: embed.add_field(name="🎵 Đang phát", value=now.title, inline=False)
        if queue:
    ...
    async def nowplaying(self, interaction: discord.Interaction):
        ...
        if now:
            ...
            if now.thumbnail: embed.set_thumbnail(url=now.thumbnail)
            await interaction.response.send_message(embed=embed)
        else:
            await interaction.response.send_message("❌ Không có bài nào đang phát!", ephemeral=True)
...
async def setup_hook():
    ...
    print("✅ Music Cog đã được load thành công!")
